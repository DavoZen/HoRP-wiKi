 ...
 `;
    ...
    convertMarkdownToHtml(markdown) {
        ...
        if (typeof markdownit !== 'undefined') {
            ...
            // Custom internal link rendering (in-site navigation)
            md.renderer.rules.link_open = function(tokens, idx, options, env, self) {
                ...
                // Internal wiki links -> in-site navigation
                if (!href.startsWith('#')) {
                    let pagePath = decodeURIComponent(href.replace(/^pages\//, ''));
                    const exists = (typeof wiki !== 'undefined' && wiki.pages)
                        ? wiki.pages.some(p => p.path === pagePath) : true;
                    tokens[idx].attrSet('href', '#');
                    tokens[idx].attrSet('class', 'wiki-link' + (exists ? '' : '-missing'));
                    tokens[idx].attrSet('onclick', "wiki.loadPage('" + pagePath.replace(/'/g, "\\'") + "'); return false;");
                    if (title) tokens[idx].attrSet('title', title);
                    return self.renderToken(tokens, idx, options);
                }
            ...
            renderer.link = function(href, title, text) {
                ...
                // External links (http/https, github) open in new tab
                if (href.startsWith('http') || href.includes('github.com')) {
                    let finalHref = href;
                    if (href.includes('github.com')) {
                        finalHref = href
                ...
                // Internal wiki links (relative paths to pages) -> in-site navigation
                if (!href.startsWith('#')) {
                    let pagePath = decodeURIComponent(href.replace(/^pages\//, ''));
                    const exists = (typeof wiki !== 'undefined' && wiki.pages) ? wiki.pages.some(p => p.path === pagePath) : true;
                    const label = exists ? '' : ' data-missing="true"';
                    return `<a href="#" class="wiki-link${exists ? '' : '-missing'}" onclick="wiki.loadPage('${pagePath.replace(/'/g, "\\'")}'); return false;"${titleAttr}${label}>${text}</a>`;
                }
